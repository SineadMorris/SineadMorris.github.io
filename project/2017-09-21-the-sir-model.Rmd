...
In class we covered the SIR model with births and deaths.
...
ggplot(SIRmod, aes(x = time, y = value, 
                   colour = as.factor(variable))) + geom_line(size = 1.5) +
...
Therefore the *number* of new births that are vaccinated (per unit time) is $\mu N p$, and the number unvaccinated is $\mu N (1 - p)$.
